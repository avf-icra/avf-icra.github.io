...
// `/avf-viewer/`. Keeping this mapping in one place lets every fetchJSON
...
function episodeStageValue(task, episode, stage) {
  const stages = episode && episode.stages ? episode.stages : {};
  if (task === "pot place" && stage === "task_success") {
    return Boolean(stages.suboptimal_place || stages.optimal_place);
  }
  return Boolean(stages[stage]);
}

function displayedStageTotal(data, task, stage) {
  if (task === "pot place" && stage === "task_success") {
    return data.episodes.reduce(
      (total, episode) => total + Number(episodeStageValue(task, episode, stage)),
      0
    );
  }
  return data.stage_totals[stage];
}
...
async function renderEpisodes(policy, task) {
  ...
  for (const stage of data.stages) {
    const total = displayedStageTotal(data, task, stage);
    out.push(`<th>${escapeHtml(displayLabel(stage))}<div class="stage-total">${total}/${data.n_total}</div></th>`);
  }
  ...
  for (const ep of data.episodes) {
    ...
    for (const stage of data.stages) {
      const val = episodeStageValue(task, ep, stage);
  ...
  if (visibleSummary.length > 0) {
    ...
    for (const row of visibleSummary) {
      ...
      for (const stage of data.stages) {
        const total = displayedStageTotal(data, task, stage);
        const value = task === "pot place" && stage === "task_success" && row.label === "raw_rate"
          ? `${(total / data.n_total).toFixed(2)} (${total}/${data.n_total})`
          : row.values[stage] || "";
        out.push(`<td>${escapeHtml(value)}</td>`);
      }
      out.push('</tr>');
    }
    out.push('</tfoot>');
...
async function renderVideo(policy, task, episode) {
  ...
  if (epRow) {
    ...
    for (const stage of epData.stages) {
      const v = episodeStageValue(task, epRow, stage);
...
let tpMode = (() => {
  try { return localStorage.getItem("avf-tp-mode") || "box"; } catch (_) { return "box"; }
})();
...
async function renderThroughput() {
  ...
  if (sel) {
    ...
    sel.addEventListener("change", () => {
      tpMode = sel.value;
      try { localStorage.setItem("avf-tp-mode", tpMode); } catch (_) {}
      tpRebuildAll();
    });
  }
...
// McNemar), (2) avf's edge vs ablations/baselines, and (3) a per-task
